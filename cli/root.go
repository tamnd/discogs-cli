// Package cli assembles the discogs command tree from the discogs
...
// NewApp assembles the kit application from the discogs domain. The
...
//
// To add a command, declare it in discogs/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := discogs.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(discogs.Domain{}).Register(app)
