...
describes what users get. Pick a format with `-o`, or let discogs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
discogs <command> -o table   # aligned columns for reading
discogs <command> -o jsonl   # one JSON object per line, for piping
discogs <command> -o json    # a single JSON array
discogs <command> -o csv     # spreadsheet friendly
discogs <command> -o tsv     # tab-separated
discogs <command> -o url     # just the URL column
discogs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
discogs <command> --fields id,title,url
```
...
```bash
discogs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
discogs <command>            # a table, because this is a terminal
discogs <command> | wc -l    # JSONL, because this is a pipe
```
